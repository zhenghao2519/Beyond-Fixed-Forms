    ...
    for seg_output in tqdm(seg_outputs, desc="Projecting 2d masks to 3d point cloud"):
        ...
        for i in range(len(gronded_sam_results)):  
            ...
            visibility_mask = compute_visibility_mask_tensor(
                scene_pts, projected_pts, depth_im, depth_thresh=0.2
            )
        ...
        # save the backprojected_3d_masks before aggregation
        # os.makedirs(os.path.join(cfg.mask_3d_dir_before_aggregate, text_prompt), exist_ok=True)
        # torch.save(backprojected_3d_masks, os.path.join(cfg.mask_3d_dir_before_aggregate, text_prompt, f"{scene_id}.pth"))
