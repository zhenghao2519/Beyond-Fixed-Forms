...
def detect(
    ...
    if cfg.must_match_base_prompt:
        
        if len(phrases) != 0:
            # print(colored(f"Detected phrases: {phrases}", "green", attrs=["bold"]))
            # print(text_prompt)
            indices = [i for i, phrase in enumerate(phrases) if cfg.must_match_class in phrase]
...
def get_parser():
    ...
    parser.add_argument("--scene", type=str, required=False, help="Specific scene to evaluate")
    return parser
